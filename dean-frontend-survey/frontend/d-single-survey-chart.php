...
add_shortcode('d-dashboard-survey-chart', 'd_survey_analysis_single');
function d_survey_analysis_single(){
    global $wpdb;
...
// Survey List and Actions
    ...
    $results = $wpdb->get_results( "SELECT * FROM $table WHERE user_id = $user_id  AND id='$id' AND is_stopped = 0 AND is_active = 1" );
    // print_r($results);
    ...
    // print_r(count($submitted));
    // die();

    if(count($results) > 0 ){
  ?>
  <br><br>
  <div class="v-survey-heading">Surveys List</div>
  <?php
                  ...
                  <tr>
                      ...
                      <td>
                          ...
                          <?php if($result->is_active == 1){ ?>
                            <a href="<?php echo $url.'?survey=d-disable-survey&action=disable&id='. $result->id .'&url='. $url .'?survey=d-survey-report'?>" class="btn btn-danger">Deactivate</a>
                          <?php }
                          else{ ?>
                          ...
                          if($result->is_stopped == 0){ ?>
                            <a href="<?php echo $url.'?survey=d-stop-survey&id='. $result->id .'&url='. $url .'?survey=d-show-single-survey' ?>" class="btn btn-danger">Stop</a>
                          <?php }
                          else{
...
add_shortcode('d-disable-survey', 'd_disable_survey');
function d_disable_survey(){
    $id = $_GET['id'];
...
add_shortcode('d-stop-survey', 'd_stop_survey');
function d_stop_survey(){
    $id = $_GET['id'];
